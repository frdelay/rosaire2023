...
// variables extraites des url
  ...
  gestionEtat() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    int? pplogin = prefs.getInt('login');
    print("++ main85 pplogin : $pplogin");
    if (pplogin == null) {
      setState(() {
        urlMedit =
            "https://app2.equipes-rosaire.org/medit_liste.php?menu=medit";
      ...
      setState(() {
        ...
        localNotificationService.generate30Notifications(
            meditationNumber: notifmedit, prenom: prenom);
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            title: InkWell(
                onTap: () async {
                  await launch('https://equipes-rosaire.org');
                },
                child: Image.asset('assets/EDR-logo-blanc.png')),
